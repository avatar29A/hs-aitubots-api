...
spec name token chatid = do
    ...
    describe "/getMe" $ it "responds with correct bot's name" $ do
        response <- runAituBotClient token manager getMe

        uniq     <- liftIO UUIDV4.nextRandom
        let bot = fromRight
                Bot { botId = uniq, botName = "", botUserName = "<undefined>" }
                response
        botUserName bot `shouldBe` decodeUtf8 name

    describe "/sendMessage" $ it "responds with code 200 - empty result" $ do
        let command = mkSendMessageWithDefaults
                "CI/CD: integrations tests sendMessage"
                peer

        response <- runAituBotClient token manager $ sendCommand command
